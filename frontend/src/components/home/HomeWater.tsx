...
export function HomeWater() {
  return (
    <section
      data-section="water"
      className={`relative overflow-hidden bg-bege-natural ${homeSectionPad}`}
    >
      <div
      ...
      <div className={`${homeShell} relative`}>
        <header className="max-w-3xl lg:max-w-5xl">
          ...
          <div data-home-item className="lg:flex lg:items-center lg:gap-8">
            <h2 className={`${homeHeroTitle} mt-5 text-preto-suave`}>
              {waterCopy.title}
            </h2>
            <p className={homeCaptionAside}>{waterCopy.imageCaption}</p>
          </div>
        </header>
        ...
        >
          ...
          <div className="flex flex-col justify-center lg:col-span-7">
            <div data-home-item className={homeStatsPanel}>
              <div
                className="pointer-events-none absolute -left-12 -bottom-12 size-40 rounded-full bg-laranja-solar/10 blur-3xl"
                aria-hidden
              />

              <p className={`relative ${homePanelEyebrow}`}>Indicadores hídricos</p>
